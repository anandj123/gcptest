...
public class GmailApiDriver {
  ...
  private static final String SERVICE_ACCOUNT_JSON_FILE_PATH =
    "/anand-1-sa.json";
  private static final Logger LOG = LoggerFactory.getLogger(
    GmailApiDriver.class
  );
  ...
  private static HttpRequestInitializer getCredentialsSA(String user)
          ...
          for (Message m : messages) {
            ...
            while (true) {
              try {
                if (retry >= 3) {
                  LOG.warn(
                    "Retry failed 3 times exiting. {thread_id : " +
                    messageId +
                    "}"
                  );
                  break;
                ...
                for (MessagePart part : parts) {
                  ...
                  if (innerParts != null && innerParts.size() > 0) {
                    LOG.debug(
                      "Truncating inner parts of {size: " +
                      innerParts.size() +
                      "}"
                    );

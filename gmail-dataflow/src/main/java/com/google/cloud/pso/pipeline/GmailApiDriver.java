...
public class GmailApiDriver {
    ...
    public Map<String,String> printMessage(String user, String historyId){
        ...
        try{
            ...
            boolean found = false;
            while(retry<3){
                ListHistoryResponse response = service
                                            .users().history().list("me")                
                ...
                if (hi == null || hi.isEmpty()) {
                    retry++;
                    try{
                        int sleeptime = (2^retry) * 500; // Exponential backup logic
                    ...
                    for(Message m : messages){ 
                        ...
                        while(true){
                            try{
                                ...
                                List<MessagePart> parts = messageFull
                                                            .getPayload()
                                                            .getParts();
                                for(MessagePart part : parts){
                                    ...
                                    if(data != null && data.length > truncateSize) {
                                        LOG.debug("Truncating main part with {from_length: " 
                                                + data.length + ", to_length : " + truncateSize 
                                    ...
                                    if(innerParts != null && innerParts.size() > 0) {
                                        LOG.debug("Truncating inner parts of {size: " 
                                                    + innerParts.size() + "}");
                                        for(MessagePart innerPart : innerParts) {
                                            data = innerPart.getBody().decodeData();
                                            if(data != null && data.length > truncateSize) {
                                                LOG.debug("Truncating inner part {from_length : " 
                                                            + data.length + ", to_size : " 
                                                            + truncateSize + ", mimeType: " 
                                                            + innerPart.getMimeType() + "}");
                                                data = Arrays.copyOf(data,truncateSize);
                                                innerPart.setBody(innerPart
                                                        .getBody()
                                                        .encodeData(data));
                                            }
                                        }
